...
let TotalAmount = () => {
  if (basket.length !== 0) {
    ...
    <button onclick="purchase()" class="checkout">Comprar</button>
    <button onclick="clearCart()" class="removeAll">Eliminar carrito</button>
    `);
  } else return;
};
...
let purchase = () => {
    Swal.fire({
        title: '¿Quiere proceder a la compra?',
        showDenyButton: true,
        ...
        if (result.isConfirmed) {
            ...
            calculation();
            localStorage.setItem("data", JSON.stringify(basket));
          Swal.fire('Comprado! Su pedido está en camino', '', 'success')
